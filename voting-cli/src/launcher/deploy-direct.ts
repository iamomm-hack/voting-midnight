...
class TaggedNodeZkConfigProvider extends NodeZkConfigProvider<
  "castVote" | "endVoting"
> {
...
async function main() {
  ...
  try {
    console.log(
      "[2/6] Connecting to Midnight Preprod testnet & local Proof Server...",
    );
    ...
    console.log(
      `[3/6] Building FRESH wallet (seed: ${SEED.slice(0, 8)}...)...`,
    );
    ...
    console.log(
      " -> Waiting for DUST gas tokens to settle on-chain (this may take 30-90s)...",
    );
    let pollCount = 0;
      ...
      Rx.timer(0, 3000).pipe(
        ...
        Rx.tap((dustBal) => {
          if (pollCount % 5 === 0 || dustBal > 0n) {
            console.log(
              `    [${pollCount}/${MAX_POLLS}] Current DUST gas balance: ${dustBal}`,
            );
          }
        }),
    ...
    const zkConfigProvider = new TaggedNodeZkConfigProvider(
      config.zkConfigPath,
    );
    ...
    console.log(
      "[6/6] Generating ZK Proofs and deploying Voting contract on-chain...",
    );
